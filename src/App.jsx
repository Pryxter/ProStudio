...
function getEntryHours(entry) {
  if (entry.status === "day-off") return 0;
...
function getEntryStatusLabel(entry) {
  ...
  if (entry.status === "open") return "Turno abierto";
...
function Dashboard({ user }) {
  ...
  const openEntry = entries.find((entry) => entry.status === "open" && !entry.checkOut);
  ...
  async function handleEarlyCheckIn() {
    ...
    if (openEntry) {
      setError("Ya tienes un turno abierto. Haz Check-Out antes de crear otro.");
      return;
    }
    ...
    try {
      await addDoc(collection(db, "timeEntries"), {
        ...
        employeeName: user.displayName || user.email,
        breakMinutes: 0,
        checkIn: Timestamp.fromDate(getFixedCheckinDate()),
        checkOut: null,
        checkInType: "early-fixed",
        status: "open",
      });
    } catch {
      setError("No se pudo guardar el Early Check-In. Intentalo otra vez.");
    } finally {
      setActionLoading(false);
    }
  }
  ...
  async function handleDayOffSubmit(event) {
    ...
    const reason = dayOffReason.trim();
    if (!reason) {
      setError("Escribe la razon del Day Off.");
      return;
    }
    ...
    try {
      ...
      setError("No se pudo guardar el Day Off. Intentalo otra vez.");
    } finally {
      setActionLoading(false);
    }
  }
      ...
      <main className="dashboard" id="top">
        ...
        <section className="summary-grid" aria-label="Resumen de horas">
          ...
          <article className="metric-card">
            ...
            <strong className={openEntry ? "status-pulse status-active" : "status-pulse status-idle"}>
              {openEntry ? "Trabajando" : "Fuera"}
            </strong>
          </article>
          <article className="action-card">
            ...
            <div className="action-buttons">
              ...
              <button
                ...
                disabled={actionLoading || Boolean(openEntry)}
              ...
              <button
                className="secondary-button"
                disabled={actionLoading}
                onClick={() => setShowDayOffForm((isShowing) => !isShowing)}
                type="button"
              >
                Day Off
              </button>
        ...
        {showDayOffForm && (
          <form className="day-off-form" onSubmit={handleDayOffSubmit}>
            <label>
              Razon del Day Off
              <input
                autoComplete="off"
                ...
                placeholder="Ej. Vacation, sick day, personal day"
                type="text"
              />
            </label>
              ...
              {entries.map((entry) => (
                <article className="entry-card" key={entry.id}>
                  <div>
                    ...
                    {entry.dayOffReason && (
                      <p className="entry-note">{entry.dayOffReason}</p>
                    )}
                  </div>
                  <dl>
                    <div>
            ...
            <tbody>
              {entries.map((entry) => (
                <tr key={`print-${entry.id}`}>
                  ...
                  <td>{entry.dayOffReason || getEntryStatusLabel(entry)}</td>
                </tr>
              ))}
            </tbody>
          </table>
